...
const CreatePoll: React.FC = () => {
    ...
    await toast.promise(
      new Promise<void>((resolve, reject) => {
        createPoll(poll)
          ...
          .catch((error) => {
            setSuccessMessage('Encountered error 🤯')
            reject(error)
          })
      }),
      {
  ...
  flex items-center px-4 mb-3 mt-2 space-x-2 relative"
    ...
    absolute left-[2.5px] py-3 rounded-full px-5 w-48 z-10"
              >
                <span className="text-transparent">.</span>
              </span>
              <input
                className="bg-transparent outline-none w-full placeholder-transparent text-sm z-20"
                name="startsAt"
                type="datetime-local"
  ...
  flex items-center px-4 mb-3 mt-2 space-x-2 relative"
    ...
    absolute left-[2.5px] py-3 rounded-full px-5 w-48 z-10"
              ...
              <input
                className="bg-transparent outline-none w-full placeholder-[#929292] text-sm z-20"
                name="endsAt"
                type="datetime-local"
                value={poll.endsAt}
                onChange={handleChange}
          ...
          {submitted && (
            <div className="mt-5 text-center">
              <img src={poll.image} alt="Poll Banner" className="w-full max-w-xs mx-auto mb-3 rounded" />
